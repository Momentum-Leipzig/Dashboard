...
months_compl = c("April", "May", "June", "July", "August", "September", "October", "November", "December", "January") 
#For plots where the data for june is extrapolated

months_miss = c("April", "May", "July", "August", "September", "October", "November", "December", "January") 
...
```{r include=TRUE, fig.show='hold', out.width = c('50%','50%'), fig.cap= "Percentage of time worked from home in December 2019", cache.whatever= list(mean_HO_T1, density_HO_T1)}
...
for (i in 1:length(months_compl)) {
  #missing mean
  if (HO_Perc_rel[i,1] %in% c("May","July")) {
    HO_Perc_rel[i,4] = HO_Perc_rel[i,2]
    
  } else if (HO_Perc_rel[i,1] == "June"){
...
```{r distributions over Time, fig.show= 'hold', out.width=c('50%','50%'),cache.whatever= HO_Perc_plot}
...
```{r distributions over Time 2, fig.show= 'hold', out.width=c('50%','50%'), cache.whatever= HO_Perc_dens}
## For details on the code for the plots, please change to the "Plots"-Tab and unfold the code there. ##
...
for (i in 1:length(months_compl)) {
  #missing mean
  if (HO_Perm_rel[i,1] %in% c("May","July")) {
    HO_Perm_rel[i,3] = HO_Perm_rel[i,2]
    
  } else if (HO_Perm_rel[i,1] == "June"){
...
for (i in 1:length(months_compl)) {
  #missing mean
  if (HO_Isol_means[i,1] %in% c("May","July")) {
    HO_Isol_means[i,4] = HO_Isol_means[i,2]
    
  } else if (HO_Isol_means[i,1] == "June"){
...
HO_Sat_Equip= 
  dplyr::filter(HO_Sat_Eff,month == "April",Equip_T3 != "Missing" ) %>% 
  ...
  # Applying multipe functions to the filtered and grouped dataset, namely mean to create Sat_Mean and sd to create Sat_sd
...
r_Sat_Equip = round(
  cor(
    filter(HO_Sat_Eff,month == "April",Equip_T3 != "Missing" )$Sat, 
    filter(HO_Sat_Eff,month == "April",Equip_T3 != "Missing" )$T3_HO3),
  digits = 2)*100
...
HO_Sat_Equip_plot = ggplot(HO_Sat_Equip, aes(y= Sat_mean, x = Equip_T3, fill =Equip_T3))+
  ...
  #panel.grid.major.x = element_blak() removes vertical lines from the plot 
                    coord_cartesian(ylim = c(1,5))+
